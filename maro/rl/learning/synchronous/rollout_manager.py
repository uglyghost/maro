...
class MultiNodeRolloutManager(AbsRolloutManager):
    ...
    def collect(self, ep: int, segment: int, policy_state_dict: dict, version: int):
        ...
        while True:
            ...
            num_finishes += self._handle_worker_result(result, ep, segment, version, combined_exp_by_policy)
    ...
    def _handle_worker_result(self, msg, ep, segment, version, combined_exp):
        if msg["type"] != MsgTag.COLLECT_DONE:
            self._logger.info(
                f"Ignored a message of type {msg['type']} (expected message type {MsgTag.COLLECT_DONE})"
            )
            return 0
    ...
    def evaluate(self, ep: int, policy_state_dict: dict):
        ...
        for _ in range(len(workers)):
            ...
            env_summary_dict[worker_id] = result["body"]["env_summary"]
