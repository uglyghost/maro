...
class MultiNodeRolloutManager(AbsRolloutManager):
    ...
    Args:
        ...
        num_steps (int): Number of environment steps to roll out in each call to ``collect``. Defaults to -1, in which
            case the roll-out will be executed until the end of the environment.
        max_receive_attempts (int): Maximum number of attempts to receive  results in ``collect``. This multiplied by
            the receive timeout in ``end_point_kwargs`` (or infinity if absent) gives the upperbound for the amount of
            time to receive the desired amount of data from workers. Defaults to None, in which case the number is set
            to ``num_workers``.
    ...
    def collect(self, ep: int, segment: int, policy_state_dict: dict, version: int):
        ...
        for _ in range(self.max_receive_attempts):
            ...
            if result["type"] != MsgTag.COLLECT_DONE:
                self._logger.info(
                    f"Ignored a message of type {result['type']} (expected message type {MsgTag.COLLECT_DONE})"
                )
                continue

            if version - result["body"][MsgKey.VERSION] > self._max_lag:
                self._logger.info(
                    ...
                    f"got {result['body'][MsgKey.VERSION]}"
                )
                continue

            exp_by_policy = result["body"][MsgKey.EXPERIENCES]
            self.total_experiences_collected += sum(exp.size for exp in exp_by_policy.values())
            self.total_env_steps += result["body"][MsgKey.NUM_STEPS]

            for policy_name, exp in exp_by_policy.items():
                combined_exp_by_policy[policy_name].extend(exp)

            if result["body"][MsgKey.SEGMENT] == segment:
                self.episode_complete = result["body"][MsgKey.EPISODE_END]
                if self.episode_complete:
                    # log roll-out summary
                    if self._log_env_summary:
                        self._logger.info(f"env summary: {result['body'][MsgKey.ENV_SUMMARY]}")
                num_finishes += 1
    ...
    def evaluate(self, ep: int, policy_state_dict: dict):
        ...
        for _ in range(len(workers)):
            result, worker_id = self._endpoint.receive()
            if result["type"] != MsgTag.EVAL_DONE or result["body"][MsgKey.EPISODE] != ep:
                self._logger.info(
                    f"Ignore a message of type {result['type']} with episode index {result['body'][MsgKey.EPISODE]} "
                    f"(expected message type {MsgTag.EVAL_DONE} and episode index {ep})"
                )
                continue

            env_summary_dict[worker_id] = result["body"][MsgKey.ENV_SUMMARY]

            if result["body"][MsgKey.EPISODE] == ep:
                num_finishes += 1
